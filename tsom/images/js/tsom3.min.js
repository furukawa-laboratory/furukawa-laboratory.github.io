...
function r(b,a,d,c,h){if(0===b)return w[a][d][c][h];if(1===b)return w[d][a][c][h];if(2===b)return w[d][c][a][h]}var e,u,N,ba,w=Na.replace(/\n+$/g,"").split("\n\n\n");for(e=0;e<w.length;e++)for(w[e]=w[e].split("\n\n"),u=0;u<w[e].length;u++)for(w[e][u]=w[e][u].split("\n"),N=0;N<w[e][u].length;N++)for(w[e][u][N]=w[e][u][N].split(" "),ba=0;ba<w[e][u][N].length;ba++)w[e][u][N][ba]=parseFloat(w[e][u][N][ba]);var L=[Ra,Sa,Ta];L.forEach(function(b,a){L[a]=b.replace(/\n+$/g,"").split("\n");L[a].forEach(function(b,
c){L[a][c]=b.split(" ");L[a][c].forEach(function(b,d){L[a][c][d]=parseFloat(L[a][c][d])})})});var C=[Ua,Wa,Ya];C.forEach(function(b,a){C[a]=b.replace(/\n+$/g,"").split("\n");C[a].forEach(function(b,c){C[a][c]=b.split(" ");C[a][c].forEach(function(b,d){C[a][c][d]=parseFloat(C[a][c][d])})})});var M=[Va,Xa,Za],Ba=[[],[],[]],sa=[],X=0;M.forEach(function(b,a){for(var d in b)Ba[a][d]=d;sa[a]=b.length});var S=[-1,-1,-1],x=[Oa,Pa,Qa],Y=[d3.layout.force(),d3.layout.force(),d3.layout.force()],V=$a.replace(/\n+$/g,
